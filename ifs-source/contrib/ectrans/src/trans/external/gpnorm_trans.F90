...
SUBROUTINE GPNORM_TRANS(PGP,KFIELDS,KPROMA,PAVE,PMIN,PMAX,LDAVE_ONLY,KRESOL,PAVE_DP)
...
REAL(KIND=JPRD)   ,OPTIONAL, INTENT(OUT) :: PAVE_DP(:)
!ifndef INTERFACE
...
IF (PRESENT(PAVE_DP)) THEN
  CALL GPNORM_TRANS_CTL(PGP,KFIELDS,KPROMA,PAVE,PMIN,PMAX,LDAVE_ONLY,F%RW(1:R%NDGL),PAVE_DP=PAVE_DP)
ELSE
