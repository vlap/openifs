...
CONTAINS
SUBROUTINE GPNORM_TRANS_CTL(PGP,KFIELDS,KPROMA,PAVE,PMIN,PMAX,LDAVE_ONLY,PW,PAVE_DP)
...
REAL(KIND=JPRD)   ,INTENT(OUT), OPTIONAL :: PAVE_DP(:)
...
REAL(KIND=JPRD) :: ZPAVE_DP(KFIELDS)
...
IF( MYSETW == 1 .AND. MYSETV == 1 )THEN
  ...
  ZPAVE_DP(:)=0.0_JPRD
  DO JGL=1,R%NDGL
    ZPAVE_DP(:)=ZPAVE_DP(:)+ZAVEG(JGL,:)
  ENDDO

  PAVE(:)=REAL(ZPAVE_DP(:),KIND=JPRB)
  IF (PRESENT(PAVE_DP)) PAVE_DP(:)=ZPAVE_DP(:)
